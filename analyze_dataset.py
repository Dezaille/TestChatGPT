...
def classify_column(values: list[Any]) -> tuple[str, str, dict[str, float | None] | None]:
    non_null = [v for v in values if not is_null(v)]
    if not non_null:
        return "empty", "texte", None
    ...
    for value in non_null:
        ...
        if parsed_number is not None and not math.isnan(parsed_number):
            ...
            text_values.append(string_value)
    ...
    if bool_ratio > 0.9:
        return "bool", "booléen", None
    if numeric_ratio > 0.9:
        stats = {
            "min": min(numeric_values) if numeric_values else None,
            "max": max(numeric_values) if numeric_values else None,
            "moyenne": (sum(numeric_values) / len(numeric_values)) if numeric_values else None,
        }
        is_integer = all(float(num).is_integer() for num in numeric_values)
        return ("int" if is_integer else "float", "entier" if is_integer else "nombre décimal", stats)
    if date_ratio > 0.8:
        return "date", "date potentielle", None
    if email_ratio > 0.8:
        return "string", "email", None
    if url_ratio > 0.8:
        return "string", "URL", None

    return "string", "texte", None
...
def describe_columns(header: list[str], rows: list[list[Any]]) -> list[dict[str, Any]]:
    ...
    for idx, name in enumerate(header):
        ...
        cardinality = len({str(v).strip() for v in values if not is_null(v)})
        dtype, semantic_type, stats = classify_column(values)

        column_meta: dict[str, Any] = {
            "nom": col_name,
            "dtype_inferé": dtype,
            "type_semantique": semantic_type,
            "valeurs_non_nulles": non_null_count,
            "valeurs_nulles": null_count,
            "taux_null": round((null_count / row_count), 4) if row_count else 0,
            "cardinalite": cardinality,
        }
        if stats:
            column_meta["statistiques"] = stats
...
def describe_dataset(path: Path, encoding: str = "utf-8", separator: str = ",") -> dict[str, Any]:
    ...
    return {
        ...
        "colonnes": columns,
        "distribution_types": dict(Counter(col["type_semantique"] for col in columns)),
        "note_confidentialite": (
            "Ce rapport ne contient pas de valeurs brutes; uniquement des métadonnées "
